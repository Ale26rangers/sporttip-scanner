...
def controlla_password():
    # Controlla se l'utente è già stato autenticato in questa sessione
    ...
    if not st.session_state["autenticato"]:
        ...
        # ⚠️ CAMBIA QUESTA PASSWORD CON UNA SCELTA DA TE!
        # Sostituisci "Svizzera2026" con la password che preferisci usare dal telefono
        if st.button("Accedi ➔"):
            if password_inserita == "Svizzera2026":
                st.session_state["autenticato"] = True
                st.rerun() # Riavvia l'app mostrando il programma vero e proprio
            else:
...
# ==========================================
# Se la funzione sopra restituisce True (password corretta), si attiva lo scanner
if controlla_password():
    ...
    # Creiamo il bottone interattivo sullo schermo del telefono
    if st.button("🔄 Avvia Scansione Palinsesto"):
        
        with st.spinner("Connessione ai server delle quote in corso..."):
            
            # Recupera l'API Key in modo sicuro dai Secrets di Streamlit
            try:
                API_KEY = st.secrets["MY_API_KEY"]
            except Exception:
                # Se non hai ancora configurato i Secrets, usa la chiave di test pubblica
                ...
                for partita in partite:
                    ...
                    for market in markets:
                        if market['key'] == 'h2h':
                            ...
                            if q1 and qX and q2:
                                # Conversione matematica per stimare la doppia chance reale di Sporttip
                                quota_1X = round((q1 * qX) / (q1 + qX), 2) + 0.35
                                quota_X2 = round((q2 * qX) / (q2 + qX), 2) + 0.35
                                
                                # FILTRO MATEMATICO RIGIDO (Solo quote comprese tra 1.35 e 1.45)
